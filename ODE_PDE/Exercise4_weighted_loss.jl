...
function loss_neuralode(p)
    ...
    S_loss = sum(abs2, true_vals[1, :] .- pred[1, :])
    I_loss = sum(abs2, true_vals[2, :] .- pred[2, :])
    R_loss = sum(abs2, true_vals[3, :] .- pred[3, :])
    return S_loss + 10* I_loss + R_loss  # Apply more weight to I_loss
end
...
for epoch in 1:10  # Run for a few epochs
...
##############result_neuralode2 = Optimization.solve(optprob_bfgs, Optim.BFGS(); callback, allow_f_increases = true,maxiters = 5000)
